...
export const EditSessionModal = ({ isVisible, onClose, session }: Props) => {
  ...
  const getInitialClubId = () => {
    if (session && session.club_id !== undefined) return session.club_id;
  ...
  useEffect(() => {
    if (session && isVisible) {
      ...
      setSelectedClubId(
        session.club_id !== undefined ? session.club_id : getInitialClubId(),
      );
    }
  }, [session, isVisible, isAdmin]);
  ...
  const handleUpdate = () => {
    ...
    if (
      ...
      Toast.show({
        ...
        text2: "Title, venue, and a valid club context are required.",
      });
      return;
    }
  ...
  const clubOptions = isAdmin
    ? [
        { id: null, name: "General Assembly", logo_url: null },
        ...availableClubs,
      ]
    : availableClubs;
