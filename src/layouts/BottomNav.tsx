...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabPress }) => {
  const tabs: { name: Tab; icon: any; activeIcon: any }[] = [
    ...
    { name: 'Attendance', icon: 'account-check-outline', activeIcon: 'account-check' },
    { name: 'Clubs', icon: 'school-outline', activeIcon: 'school' },
    { name: 'Events', icon: 'calendar-check-outline', activeIcon: 'calendar-check' },
  ];

  const containerPadding = Platform.OS === 'ios' ? 'pb-8 pt-3' : 'py-3';

  return (
    <View className={`flex-row bg-background dark:bg-dark-bg border-t border-border dark:border-dark-border justify-around items-center ${containerPadding}`}>
      {tabs.map((tab) => {
